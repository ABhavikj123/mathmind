...
export default function GamePage() {
  ...
  useEffect(() => {
    ...
    if (storedName) {
      ...
      setGameState("welcome") // Show welcome back screen
    } else {
      setGameState("start") // Show start screen to enter name
    }
  ...
  const handleWelcomeComplete = () => {
    // Use a callback to ensure state updates happen in the correct order
    setTimeout(() => {
      setGameState("playing")
    }, 0)
  }
  ...
  return (
    <div className="flex flex-col md:flex-row h-[calc(100vh-64px)]">
      {/* Game sidebar - hidden by default on mobile */}
