...
export default function ProblemDisplay({ expression, operands, operators }: ProblemDisplayProps) {
  // Create a formatted expression with implied brackets
  const formattedExpression = operands.reduce((acc, operand, index) => {
    if (index === 0) return operand.toString()

    // Add brackets around previous result for visual clarity
    if (index === 1) {
